...
# ★ 历史留痕排除（2026-09-24 · `云内核-C3-004` 立 · **`C3-005` 收窄**）：**述及"曾经 🟡"的历史陈述**
#   **不是"仍在挂起"的 🟡** ⇒ 排除。★ 依 `T-041`：**剔除规则本身是正式规则** ＋ **每次扫描打印剔除行数**。
#   ★ **收窄口径（用户裁定 2026-09-24）**：**只留 2 词**「原文保留」「保留留痕」—— **排除越宽，隐身风险越大**；
#     收窄后**个别历史句会复现告警**，那**是正确现象**（宁可多报，不可漏报）。
HIST_MARKS = ("原文保留", "保留留痕")
