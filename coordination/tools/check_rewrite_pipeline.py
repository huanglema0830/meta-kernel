...
【口径（五条）】
  ...
  2. **受检对象**：「**最后一个含「解冻」的节**」——
     ...
     ★★ **`C3-012` 裁定①（收窄近似面）**：原口径为「**含『解冻』或 `D16'`**」——
        **`D16'` 单独出现不再使该节成为受检节**（`"仅引述 D16'" ≠ "当轮走过解冻程序"`；
        实证：`BASELINE §77.3` 只**引述**「不经 `D16'`」而被判「半冻」，见 `§78.6-②`）。
        ★ **原近似口径留痕（`C19`）**：不回改历史登记，差异在当轮报告登记。
...
【自检】
  `--selftest`：正反对照**九侧** ——
    ...
    ⑨ ★ **`C3-012` 裁定①**：**只引述该程序名、全文无「解冻」** ⇒ **不设为受检节**（反例 · 收窄近似面）
...
def pick_target(sections):
    """**最后一个含「解冻」的节**索引；无则 None。

    ★★ `C3-012` 裁定①：原实现用「含『解冻』**或** `D16'`」近似"做过回写的节" ——
    **仅引述 `D16'`**（如实录一句"本项不经该程序"）即会被误选为受检节 ⇒ 假告警。
    ⇒ **收窄为只看「解冻」**（`D16'` 只作段③留痕的命中词之一，不再<｜hy_place▁holder▁no▁813｜>选节条件）。
    """
    hit = None
...
def evaluate_sections(sections):
    ...
    if k is None:
        notes.append("台账中**未找到含「解冻」的节** ⇒ 本判据**当期不适用**（非告警）")
        return alerts, notes, None
    ...
    # B 半冻检测（★ `C3-012` 裁定①：与 `pick_target` 同口径收窄 —— **只看「解冻」**）
    if ("解冻" in body) and not any(x in body for x in MARK["⑤重冻"]):
...
def selftest():
    ...
    # ★ `C3-012` 裁定① 后 `pick_target` 只认「解冻」 ⇒ 本夹具**须含「解冻」**（否则根本不会被选中，
    #   测不到"豁免出口"这条路径）—— 夹具据此调整，**语义不改**（仍＝"被选中但因豁免标记而不受五段要求"）。
    skip_fixture = ("## 八十八、纯出稿轮\n"
                    "> 本节**只出稿、未改底图**；只议了是否解冻，实际未解冻（**非回写轮**）\n"
                    + SKIP_MARK + "\n")
    ...
    # 侧⑨ ★ `C3-012` 裁定①（收窄近似面）：**只引述 `D16'`、全文无「解冻」** ⇒ **不设为受检节**（反例）
    #   实证：`BASELINE §77.3` 曾因一句"不经 `D16'`"被判「半冻」（`§78.6-②`）。
    quote_only = ("## 八十九、只引述不执行的节\n"
                  "> 本节记载：本次口径修订属**治理修订**，不经该底图回写程序（只是**引述**）。\n")
    a3, n3, t3 = evaluate_sections(split_sections(quote_only))
    ok = (not a3) and any("当期不适用" in x for x in n3)
    print("  侧⑨（反例·**只引述**、无「解冻」⇒ 不设为受检节〔`C3-012` ①〕）: %s%s"
          % ("PASS" if ok else "FAIL", "" if ok else "  ← %s" % (a3[:1] or n3[:1])))
    if not ok:
        fails.append("侧⑨")
    ...
    if fails:
        ...
        return 1
    print("自检结论：PASS（**九侧**：3 正例 ＋ 5 反例 ＋ 1 对照）")
    return 0
...
def main():
    ...
    print("仓库根：%s" % repo)
    print("★ 受检对象＝台账**最后一个含「解冻」的节**（回看 %d 节供段①②）" % LOOKBACK)
    print("★ `C3-012` 裁定①：原「含『解冻』或 `D16'`」已收窄 ⇒ **`D16'` 单独出现不再使该节受检**")
    print("=" * 66)
