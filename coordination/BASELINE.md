...
| R2 | 推送**间歇性连接超时**——旧称"SNI 摩擦"，**该命名不准确**：实测为 **TCP/连接层偶发超时**，且与 **TLS 重协商**相关（非 SNI 拦截） | 🟡 **本轮已完成专项排查**（见 `reports/2026-09-16_R2推送连接超时根因报告.md`）：已排除 SNI 拦截／代理／hosts 劫持／证书伪造；TCP 12/12＋并发 8/8＋持续探针 6/6 通过，`git` 协议层偶发失败（1/20）。**对策首选 SSH over 443**（通道已验证可达，仅缺本机 key —— **配置动作待用户确认**）；现有重试脚本为有效缓解 |
...
| **D19** | R2 的对策：切换为 **SSH over 443** | 🔄 **已启动执行**（用户已决策"做"）：专用 ed25519 key 已生成（`~/.ssh/id_ed25519_metakernel`，指纹 `SHA256:lrDGeu3NSENVUHjA4TrVEWOVeTzZsG7BCkXu+qhFAvY`）＋ `~/.ssh/config` 已配 `github-443` 别名（含保活参数）＋ 一键切换脚本 `deploy/github_switch_to_ssh.sh`（幂等、失败自动回退）。**当前阻塞点：公钥尚未添加到 GitHub 账号** ⇒ **不会在公钥生效前切换 remote**（否则本轮推送会中断）；待用户添加后一键执行并验证 |
