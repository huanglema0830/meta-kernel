...
| **私有资产盘点缺失** | ✅ **已建立**（D30）：`coordination/ASSETS_INVENTORY.md`＝**指针**（公开安全）；**全量清单（39 项 × 五类 × 四级别 + 原术语↔物理语言对照表）存于仓库外本地目录**——因为**本仓库为 PUBLIC**（见 **R20**），把清单写进仓库等于公开资产与换壳对照表。触发词「资产盘点」／「语言替换」；已登记 `CHARTER.md` 机制 **16** |
| L6感知、L7执行器 | ⏳ P2 |
| 覆盖率、知识库 | ⏳ P2 |
...
| **D30** | **本仓库为 PUBLIC**（R20 发现的处置）+ 私有资产清单的落点 | ⏳ **待用户决策（本轮新增，P0）**：实测仓库 `"visibility":"PUBLIC"`、匿名 `curl` 取仓库内文件 **HTTP 200** ⇒ **命名体系／机制条文／约束全文／阈值符号与中性点口径已在公网**。<br>**本轮已做的止损（不等确认、可逆、不破任何纪律）**：私有资产清单**改存仓库外本地目录**，`coordination/ASSETS_INVENTORY.md` 只放指针 ⇒ **资产分级与换壳对照表未公开**。<br>**待定选项（六要素见本轮报告 §四）**：① 保持现状、资产清单永久留本地（★建议）② 仓库转 private（**会影响公开站点，不可逆**）③ 按原指令把清单写进 `coordination/`（**不推荐**）;<br>另需你定 **语言替换的档位**（A 全替换／B 只换对外可见面／**C 只做增量换壳（★建议）**／D 暂不替换） |
