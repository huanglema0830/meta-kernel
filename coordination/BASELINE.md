...
| **D27** | **A-2 的精确范围**（实测后与预期"~15"有差） | ⏳ **待你确认**（清单已出）：实测结论是 **12 模块可干净迁移**（7 零改造 + 5 需拆分），另 **6 模块建议留 2.3**——`fourier`/`interference`/`energy`/`l5_baseline`/`l5_quad`/`l5_evidence`，**其 alloc 在算法内部或结构体字段**，强拆会改算法语义（违反"只拆接口不拆算法"）。**建议接受"12 + 6 分期"**。前置 `rustup target add x86_64-unknown-none` ✅ **已完成** |
