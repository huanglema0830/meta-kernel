...
最后更新于：HEAD e85ddca（对应 v0.153）｜本轮（C11 七要素 + 优先级纳入 + 私有资产盘点 + D28/D29 收口 + **R20 仓库可见性**）更新内容见下方（本次更新随提交计入下一版）
更新时间：2026-09-16
...
| **D31** | **子任务2.2 引导器选型**（本轮新增） | ⏳ **待用户确认（清单已出）**：建议自"GRUB/Limine"**改口径为 `bootloader` 0.11**——官方 v0.9 迁移文档明写内核用 **`x86_64-unknown-none` 即可，不需要自定义 JSON target、不需要 build-std**（2.1 成果**直接复用**）。<br>**三个必答子项**：**a** `bootloader_api` 只进 boot 层、内核与 nostd crate 保持零依赖（C1 边界解释）｜**b** 引入 **nightly** 且**只在 `meta-kernel-boot/` 内生效**（实测本机仅有 stable；`bootloader` 0.11 官方要求 nightly + `llvm-tools-preview`）｜**c** QEMU 放 **CI**（★）还是本机。<br>**验证路线**：**A 帧缓冲 + QEMU screendump（零 unsafe，★推荐）**／B 串口（需 C9 放行 1 处 port I/O）。<br>**清单**：`reports/2026-09-16_子任务2.2_改动清单_待确认执行.md`（7 新文件 + 2 处改动；**本轮未改任何代码**） |
