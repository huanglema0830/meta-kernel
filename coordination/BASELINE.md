...
最后更新于：HEAD 9b3d9fc（对应 v0.159）｜本轮（**子任务2.2 执行完成**：bootloader 0.11 接入 + QEMU 真实引导 + 截屏断言；新增 R21/R22）更新内容见下方（本次更新随提交计入下一版）
更新时间：2026-09-16
...
| **R20** | **本仓库为 PUBLIC，`coordination/` 全部 45 个文件**（含 BASELINE/CHARTER/CONSTRAINTS/ROADMAP/advisor_brief 等）**可被任何人匿名读取** —— 实测：`gh repo view --json visibility` → `"PUBLIC"`；**匿名 `curl` 取仓库内文件返回 HTTP 200**；Pages 站点亦 `"public": true`。⇒ 完整**命名体系、层级架构、16 项机制条文、C1–C12 全文、R/D 编号与描述、阈值符号与中性点口径**均**已在公网** | ⚠️ **本轮发现，未处置（待用户决策 D30）**：这属**用户决策级**事项（是否改可见性会**不可逆地影响已建站点**，按 C10 不得自行变更）。**本轮已做的止损**：① **不**把「私有资产清单 / 分级 / 换壳对照表」写进仓库（改存仓库外本地目录，见 `ASSETS_INVENTORY.md` 指针）② 在本轮报告里按六要素上报。**注**：git 历史中已推送的原术语**无法撤回**（改写历史＝不可逆且会破所有克隆） |
...
全文见 `CONSTRAINTS.md`；一句话速查见 `CHARTER.md` §四。**十二条**摘要：
