...
| **CI 覆盖 field-render 单元测试（R18 遗留 / D-a）** | ✅ **已补**：`host-windows` 新增「单元测试 field-render」步骤（显式断言 ≥30 passed，防空转）；本地原样干跑 **35 passed / exit=0** |
| L6感知、L7执行器 | ⏳ P2 |
| 覆盖率、知识库 | ⏳ P2 |
...
| **D19** | R2 的对策：是否切换为 **SSH over 443** | ⏳ **待用户确认**：涉及在 GitHub 账号添加 SSH 公钥（**账号级配置变更**，按 C10 须先确认）。通道已实测可达（`ssh.github.com:443` 能到认证阶段，仅缺本机 key） |
| **D20** | 通用工作流的**算法层**是否/何时填充 | ⏳ 待定：当前算法层**全部为占位**（保密层）；填充时机与方式由用户决定 |
