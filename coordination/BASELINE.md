...
| **D22** | 决策评估机制（顾问侧六要素） | ✅ **已建立**（2026-09-16 用户决策）：新增 **C11** + `TEMPLATES.md` **§九 决策评估模板**——顾问凡涉及决策必须给六要素（**评估／优势／劣势／风险／建议／等待用户决策**），优势/劣势/风险各 ≥2 条且**具体到可验证**，风险标注概率与影响，选项 >2 时附「各选项对比」表；**用户决策前顾问不得擅自执行**。已同步 CHARTER（C11 + 机制第 14 项）/CONSTRAINTS/CONSTRAINTS 活相清单；🔬 试用中 |
