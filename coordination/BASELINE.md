...
最后更新于：HEAD 947bf1b（对应 v0.171）｜本轮（**2.3b 分片启动（片1 已执行并由 CI 真验）＋ alloc 裸机门禁通过 ＋ 机制17 第二实例（大模型接入骨架）＋ C13 扩为 9 步 ＋ 新增 C15/R25/R26**）更新内容见下方（本次更新随提交计入下一版）
更新时间：2026-09-17
更新时间：2026-09-17
...
| **D30-1** | Git 历史是否追改（**2026-09-17 用户确认**） | ✅ **选路径 A：接受现状**。**用户给出的四条理由**：① 已推送的内容**只是骨架**（机制条文／术语体系／模板结构）② **真正需要保密的**（算法实现、具体阈值、基因库内容）**从未进入公开仓库** ③ **`git filter-repo` 的风险远大于收益**（所有克隆失效、fork 保留、重新污染）④ **C 档增量换壳**已确保**未来不再新增暴露**。<br>⇒ **不追改历史、不做 force push、不重写历史**。处理原则已写入 §八「**已推送内容的处理原则**」；R20 已按此更新为"保持未解除 + C 档 + 接受现状" |
