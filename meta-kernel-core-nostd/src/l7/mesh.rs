...
//! 【2.3b 片2 迁移】与 `meta-kernel-core/src/l7/mesh.rs` **同源**；仅作下述适配，其余**逐字未改**：
//! ① 补 `alloc`/`core` 的 `use`（no_std 下 `Vec`/`String`/`vec!`/`format!`/`ToString` 不在 prelude）
//!    —— `vec!` 本文件**只在 `#[cfg(test)]` 内使用** ⇒ **不引入** `use alloc::vec`（引入反成未使用告警）
//! ② `std::` 路径 → `core::`/`alloc::`（同一常量/同一类型）
