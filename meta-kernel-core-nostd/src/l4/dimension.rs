...
impl FieldState {
    ...
    // ⚠️ 拆分说明（路径 A-2）：
    // 原 std 版有 `to_vec(&self) -> Vec<f64>` 与 `from_vec(&[f64]) -> Option<Self>`
    // —— 它们是**对外接口载体**（需要 `alloc`），**已留在 `meta-kernel-core` 一侧**。
    // `no_std` 侧改用**固定长数组**，语义等价、零分配。

    /// → `[f64; 7]`（对外接口载体；无分配）。
    #[must_use]
    pub fn to_array(&self) -> [f64; 7] {
        [self.t, self.f, self.a, self.phi, self.x, self.entropy, self.tau]
    }

    /// ← `[f64; 7]`（长度固定，恒成功）。
    #[must_use]
    pub fn from_array(v: [f64; 7]) -> Self {
        Self::new(v[0], v[1], v[2], v[3], v[4], v[5], v[6])
    }

    /// ← `&[f64]`（长度必须为 7；保留原 `from_vec` 的**校验语义**，但不分配）。
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn struct_fields_roundtrip_array() {
        let st = FieldState::new(0.5, 1.2, 3.0, 0.9, 2.2, 1.4, 0.8);
        let v = st.to_array();
        assert_eq!(v.len(), 7);
        let back = FieldState::from_slice(&v).expect("7 元素可还原");
        assert_eq!(back, st);
        assert_eq!(FieldState::from_array(v), st);
    ...
    fn from_slice_rejects_wrong_length() {
        assert!(FieldState::from_slice(&[1.0, 2.0]).is_none());
        assert!(FieldState::from_slice(&[]).is_none());
        assert!(FieldState::from_slice(&[1.0; 8]).is_none());
    }
