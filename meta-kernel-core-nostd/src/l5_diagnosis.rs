...
//! 【2.3b 片6 迁移】与 `meta-kernel-core/src/l5_diagnosis.rs` **同源**；仅作下述适配，其余**逐行逐字未改**：
//! ① 补 `alloc` 的 `use`（no_std 下 `String`/`String`/`Vec` 不在 prelude）
//! ② 无 `std::` 路径需改写（本文件零 `std::` 用法）
//! ③ 引入 `FloatOps` trait ⇒ `.abs()` 在 no_std 下解析到 `fmath`（**调用点一行未改**）
