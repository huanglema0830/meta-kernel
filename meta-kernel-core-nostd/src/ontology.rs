...
//!
//! 层级总表：
//! 0 黑（未识/真空）→ 1 赤（原初粒子/最小扰动）→ 2 橙（二元/约束）→ 3 黄（结构/语法）
//! → 4 绿（组合/组织）→ 5 青（自循环/递归）→ 6 蓝（呈现/形态）→ 7 紫（系统/超个体）
//! → 8 穿透（空间/场/拓扑）→ 9 留白（时间/方向/不可逆）→ 10 白（圆满/照明）。

//! 【2.3b 片3 迁移】与 `meta-kernel-core/src/ontology.rs` **同源**；仅作下述适配，其余**逐行逐字未改**：
//! ① 补 `alloc`/`core` 的 `use`（no_std 下 `Vec`/`vec!`/`String`/`ToString`/`format!` 不在 prelude）
//! ② `std::f32::consts::PI` → `core::f32::consts::PI`（同一常量）
//! ③ `std::collections::BTreeMap` → `alloc::collections::BTreeMap`（**std 只是重导出同一类型** ⇒ 零语义变化）
//! ④ `std::collections::HashSet` → `alloc::collections::BTreeSet`（no_std 无 HashSet；**两处只用 insert/len、从不迭代** ⇒ 逐位等价；编译级证据见报告）
//! ⑤ 引入 `FloatOps` trait ⇒ 浮点方法在 no_std 下解析到 `fmath`（调用点一行未改；**f64 侧同日实现**）
use alloc::vec::Vec;
use alloc::vec;
...
pub fn feature_scores(p: &Pattern) -> Vec<f64> {
    ...
    let mut levels_used = alloc::collections::BTreeSet::new();
    for e in &p.elements {
...
pub fn abstract_pattern(decomposed: Vec<Element>) -> AbstractSchema {
    let mut counts = alloc::collections::BTreeMap::<u8, u32>::new();
    for e in &decomposed {
