...
//! 【2.3b 片6 迁移】与 `meta-kernel-core/src/positive_source.rs` **同源**；仅作下述适配，其余**逐行逐字未改**：
//! ① 补 `alloc` 的 `use`（no_std 下 `Vec` 不在 prelude）
//! ② **唯一的类型替换**：`std::collections::HashMap` → `alloc::collections::BTreeMap`
//!    —— 成立前提＝该集合**只用 `insert`/`get`、从不迭代**（独立复核：方法集恰为 {insert, get}；
//!    `PositiveSource` derive 为 `Debug, Clone, Default`，**无 `PartialEq`/`Eq`/`Hash`** ⇒ 遍历序不可观测）。
//! ③ ★ **本片唯一一处「改文字」**：**3 行 `O(1)` 注释 → `O(log n)`**（行 35／337／409）。
//!    理由：`BTreeMap::get` 是 `O(log n)`，`HashMap` 才是均摊 `O(1)` ⇒ 换容器后原注释成为不实叙述。
//!    ⚠️ **旧判据抓不到它**（保真度判据只证「没多改字」，不证「改了的那字对」）
//!    ⇒ 故**显式登记在 `check_migration_fidelity.py::ALLOWED_SUBS`** 作为**可追溯项**（3 条）。
//!    ⚠️ **`meta-kernel-core`（std 侧）保持 `HashMap` 不变 ⇒ 那边 `O(1)` 依然为真，故不改源文件。**
//! ④ 引入 `FloatOps` trait ⇒ `.log2()` / `.abs()` 在 no_std 下解析到 `fmath`（**调用点一行未改**）
//! ⑤ 本片清单**由 `--emit 1` 产出**（D40），**不手写**
use alloc::vec::Vec;
...
/// 前者用于常规检索，后者用于 O(log n) 直接配对（非遍历）。
...
pub struct PositiveSource {
    ...
    /// 孪生索引：twin_fingerprint → entangled 下标（O(log n) 直接配对，非遍历）。
    twin_index: BTreeMap<u64, usize>,
...
impl PositiveSource {
    ...
    /// 心流凿空：直接按输入指纹的孪生配对查找（O(log n)，非遍历）。
