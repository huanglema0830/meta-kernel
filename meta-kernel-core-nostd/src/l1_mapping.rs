...
//! 【2.3b 片6 迁移】与 `meta-kernel-core/src/l1_mapping.rs` **同源**；仅作下述适配，其余**逐行逐字未改**：
//! ① 补 `alloc`/`core` 的 `use`（no_std 下 `String` 不在 prelude）
//! ② `std::` 路径 → `core::`（同一常量，**零语义变化**：`std::f64::consts::{PI,TAU}` → `core::f64::consts::*`）
//! ③ 引入 `FloatOps` trait ⇒ `.round()` / `.abs()` 在 no_std 下解析到 `fmath`（**调用点一行未改**）
//! ④ 本片清单**由 `coordination/tools/check_migration_closure.py` 的 `--emit 1` 产出**（D40），**不手写**
use alloc::format;
...
pub fn field_to_gabor_with(f: &FieldReading, lib: &GeneLibrary) -> GaborParams {
    ...
    GaborParams {
        lambda: safe_pos(core::f64::consts::TAU / (e * c[0] + safe_pos(c[1], 1e-9)), 1e-6),
        theta: wi * c[2] * core::f64::consts::PI,
