...
/// 已补 `r¹⁴/14!`。
/// ⚠️ **2026-09-20（§7.6 补丁稿 · 第 2 步 · D-1 裁定）**：`sin` 原先止于 `r¹³/13!`
/// （首个被丢掉的项 `r¹⁵/15! ≈ 1.56e-14`，是补完 `cos` 之后**残差的主因**），已补 `r¹⁵/15!`。
/// ⚠️ **大参数区（`|x| ≳ 1e3`）的归约误差仍未修**（需 Payne-Hanek，见
/// `sin_cos_f64_large_argument_report` 的实测数字，**该区段不宣称达标**）。
fn sin_cos_f64(x: f64) -> (f64, f64) {
    ...
    let r2 = r * r;
    // sin(r) = r(1 - r²/6 + r⁴/120 - r⁶/5040 + r⁸/362880 - r¹⁰/39916800 + r¹²/6227020800 - r¹⁴/1307674368000)
    // ⚠️ 2026-09-20（§7.6 补丁稿 · 第 2 步 · **D-1 裁定「先补 `sin` 一项」**）：
    //    **补上 `r¹⁵/15!`（`15! = 1_307_674_368_000`）** —— 在 Horner 内层表现为 `r2^7` 系数 `-1/15!`
    //    （提取了一个 `r`，故写 `r¹⁴` 系数；它与外层的 `r` 相乘即 `r¹⁵/15!`）。
    //    改前 `sin` 止于 `r¹³/13!`，在 `|r| = π/4` 处**首个被丢掉的项** = `r¹⁵/15! ≈ 1.56e-14`
    //    ⇒ **它就是补完 `cos` 之后残差的主因**（实测主区间仍在 **184 ULP** 量级，D29 的 ≤4 ULP 未达）。
    //    改后 `sin` 首个被丢掉的项降到 `r¹⁷/17! ≈ 4.7e-17`。
    let s = r
        * (1.0
            + r2 * (-1.0 / 6.0
                + r2 * (1.0 / 120.0
                    + r2 * (-1.0 / 5040.0
                        + r2 * (1.0 / 362_880.0
                            + r2 * (-1.0 / 39_916_800.0
                                + r2 * (1.0 / 6_227_020_800.0 + r2 * (-1.0 / 1_307_674_368_000.0))))))));
...
mod tests {
    ...
    ///   - `sin` 级数现在到 **`r¹⁵/15!`**（2026-09-20 · D-1 补项；此前止于 `r¹³/13!`）
    ///     ⇒ **首个被丢掉的项** = `r¹⁷/17!`；
    ///   - `cos` 级数现在到 **`r¹⁴/14!`**（2026-09-19 第 1 步补；此前止于 `r¹²/12!`）
    ///     ⇒ **首个被丢掉的项** = `r¹⁶/16!`。
    ///   - 归约后 `|r| ≤ π/4`（Cody-Waite 的经典界）⇒ 取 `rmax = π/4` 代入。
    ///   ⚠️ **每次再补一项，本处的两个"首个被丢掉的项"必须同步改**（否则期望值停留在旧级数上，
    ///      判据会把"已经补好的实现"判红 —— 即"断言失败先自查期望值"）。
    ...
    /// ⇒ **两个函数的误差界都必须取"两者较大者"**（现为 `r¹⁶/16!`，来自 `cos` 级数），
    /// 而**不是**"`sin` 只受 `r¹⁷/17!` 约束、`cos` 只受 `r¹⁶/16!` 约束"。
    /// 初版按后者设界 ⇒ **实测判红**；核对象限表后确认**是期望值写错、不是补丁写错**
    /// （"断言失败先自查期望值"）⇒ 已改正。
    ///
    /// ★ **阳性对照（两步，数字都要进报告，R35：报数写口径）**：
    ///   ① **第 1 步（`cos` 补 `r¹⁴/14!`）前**：本测**必须失败**（实测 `cos` `3.889e-13`，≈ 该界的 **95 倍**）；
    ///      施加后通过（实测 `2.043e-14`）。
    ///   ② **第 2 步（`sin` 补 `r¹⁵/15!`）前**：残差主因是 `r¹⁵/15! ≈ 1.56e-14`
    ///      ⇒ 实测应**贴近 1.56e-14**（实测 `2.043e-14`）；补完后应降到 **`r¹⁶/16! ≈ 1.0e-15`** 量级。
    #[test]
    fn sin_cos_f64_truncation_bound() {
        let rmax = core::f64::consts::FRAC_PI_4; // π/4 ≈ 0.7853981633974483
        let mut p = 1.0f64;
        let mut last_omitted_sin = 0.0f64; // rmax^17 / 17!（sin 已补到 r¹⁵/15!）
        let mut last_omitted_cos = 0.0f64; // rmax^16 / 16!（cos 已补到 r¹⁴/14!）
        for k in 1..=17u32 {
            p *= rmax;
            if k == 17 {
                last_omitted_sin = p / factorial_f64(17);
            }
            if k == 16 {
        ...
        println!(
            "[fmath·f64] 理论界：rmax=π/4 | 首个被丢掉项 sin(r¹⁷/17!)={:.4e} cos(r¹⁶/16!)={:.4e} | 统一界(2×max)={:.4e}",
        ...
        assert!(
            ss.max_abs <= bound,
            "f64 sin 截断误差 {:.4e} 超出界 {:.4e}（=2×max(r¹⁷/17!, r¹⁶/16!)）最差点 {}",
            ss.max_abs,
            bound,
            ss.worst_abs_x
        );
        assert!(
            cs.max_abs <= bound,
            "f64 cos 截断误差 {:.4e} 超出界 {:.4e}（=2×max(r¹⁷/17!, r¹⁶/16!)）最差点 {}",
            cs.max_abs,
            bound,
            cs.worst_abs_x
        );
