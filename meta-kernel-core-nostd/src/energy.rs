// =====================================================================
// 【L1·戒律·解构】不杀生=结构转化不消灭：低活力(e≤0.2)解构到原初粒子、中低活力进
// 拆解-重编循环；不饮酒=演化仅允许 加法/×0.618/e 衰减 三类算子（见 decay 常量）。
// 纯注释标注，零逻辑改动。
// =====================================================================
//! # 能量层级判定（Energy）
//!
...
//! | 活力指数 e | 处置 |
//! |---|---|
//! | e ≤ 0.2 | 直接分解到原初粒子（层级 1） |
//! | 0.2 < e ≤ 0.5 | 进入"拆解-分析-重编"循环 |
//! | 0.5 < e ≤ 0.8 | 保留结构，标记"观察中" |
//! | e > 0.8 | 直接纳入正源库 |

//! 【2.3b 片3 迁移】与 `meta-kernel-core/src/energy.rs` **同源**；仅作下述适配，其余**逐行逐字未改**：
