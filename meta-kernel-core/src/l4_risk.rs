...
/// 人读解释（供 L6 呈现；不产生 HTML）。
pub fn explain(v: RiskVerdict, a: &RiskInput) -> String {
    match v {
        RiskVerdict::Cleared => format!("四条戒律皆过，可动：{}", a.name),
        RiskVerdict::SteadyFirst(p) => {
            format!("不妄动：{}未过——内在状态未稳，先稳后动（{}）", p.label(), a.name)
        }
        RiskVerdict::Refused(p) => format!("拒绝：{}未过（{}）", p.label(), a.name),
    }
}
...
mod tests {
    ...
    #[test]
    fn labels_and_explanations_are_complete() {
        for p in Precept::all() {
            assert!(!p.label().is_empty());
        }
        assert_eq!(Precept::all().len(), 4);
        let s1 = explain(RiskVerdict::Cleared, &ok_action());
        assert!(s1.contains("可动"));
        let s2 = explain(RiskVerdict::Refused(Precept::NotHarm), &ok_action());
        assert!(s2.contains("不害"));
        let s3 = explain(RiskVerdict::SteadyFirst(Precept::NotReckless), &ok_action());
        assert!(s3.contains("先稳后动"));
    }
