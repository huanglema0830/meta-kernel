...
pub mod engine_select; // 物态 → 引擎 选择（Q11 可测契约：U1=预算态／U2=标签+结果；2026-09-19 裁定落地）
pub mod l4; // L4 场域戒律判定引擎（应用框架·拒绝层 v0.002：不非时食/不捉持）
...
pub mod l3_world;      // L3 世界模型（页面/站点/交互的累积状态；画面呈现的是它的状态；v0.122）
pub mod l4_risk;        // L4 风险判断戒律（不害/不妄动/可回退/自承）挂接四元组（v0.112）
pub mod l5_quad;        // L5 四元组内在变量（紧张/平静/喜欢/安全）+ 探测策略（v0.112）
pub mod l5_evidence;    // L5 诊断证据：世界模型匹配度 + 预测误差 → 置信度依据（v0.123）
pub mod l5_attention;   // L5 注意力—渲染—探测联动（LOD 档位 + 探测门槛同源；v0.123）
