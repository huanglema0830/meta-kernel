# WebGPU 宿主 · UI 方案抉择与实施计划（HOST_UI_DECISION）v1.3

> 状态：**v1.0（2026-09-15）** ｜ 依据：发起人「第三阶段：WebGPU 宿主」§三（UI 渲染方式需我方判断）
> 结论：**选 `egui + wgpu`**；但本轮**未交付可运行的窗口宿主**，原因与下一步见 §3/§4（诚实说明）。**（★ 2026-09-21 加锚：本行为 **v1.0（2026-09-15）时点口径**；**至 v0.283 窗口宿主已交付** —— `host/field-render/src/window.rs`（`winit` `ApplicationHandler` ＋ `wgpu` surface ＋ **`egui 0.30` UI**）＋ CI **验收④「真实开窗 + URL 取源码 + 上屏回读」硬门禁**（`.github/workflows/ci.yml` L1016；已去 `continue-on-error`）；本行原句未改）**
...
| 验收项 | 现状 |
|---|---|
| 1 窗口能打开并看到场域画面 | ⏳ **未交付**（见 §3）**（★ 2026-09-21 加锚：**已过期** —— 见 §6.2 客观验收「窗口能打开 ✅ **1536×1536**（HiDPI）；三样本方差 43.9／1182.8／1370.7，非纯黑非纯白」；本行原句未改）** |
| 2 地址栏可输入 URL | ⏳ 未交付（方案已定：egui 文本框）**（★ 2026-09-21 加锚：**已过期** —— `window.rs` 有 URL 取源码（`fetch_source`，系统 `curl`）＋ CI 验收④「**URL 取源码**」硬门禁；本行原句未改）** |
| 3 多标签可切换 | ⏳ 未交付（方案已定）**（★ 2026-09-21 加锚：**已过期** —— `window.rs` `struct Tab`／`NewTab`／`CloseTab`；本行原句未改）** |
| 4 下载能触发 | ⏳ 未交付（方案已定：写 .ppm + .json）**（★ 2026-09-21 加锚：**已过期** —— `window.rs` `Action::Download`／`fn download()`（写 `field.json` ＋ `frame.ppm`）；产物实证 `host/field-render/downloads/`；本行原句未改）** |
...
- **v1.3（2026-09-15 · 喜欢重设计 + 世界模型 + 动态 LOD）**：见 §9（含度量修正过程与参数扫描）。
...
### 6.4 仍未做（如实）
地址栏 / 多标签 / 下载 / egui —— **属第三步后续步骤**，等 UI 路线裁决（§1 三方案）。**（★ 2026-09-21 加锚：本行为 **v1.1 时点口径**；**至 v0.283 四者均已交付**（`egui 0.30` ＋ 地址栏 URL 取源码 ＋ `Tab` 多标签 ＋ `download()`），见 §6.2 与 `.github/workflows/ci.yml` L1016 验收④；本行原句未改）**
...
## 9. 「喜欢」重设计 + 世界模型载体 + 动态 LOD + 老笔记本准备（v0.122）

### 9.1 「喜欢」＝预测误差的降低（自由能原理）
内核新增 `l1_mapping::{seed_coherence_into, coherence_coeffs, coherence_of}`：
`coherence = clamp(floor + like_gain·喜欢 + safety_gain·安全 + calm_gain·平静, 0, 1)`，
系数存基因库 `coherence.*`（**改库即改映射**）。宿主把一致性落到三处：
**① 位置规则度 ↑（抖动收缩）② 相位稳定 ↑（ψ 收缩）③ 包络展宽 ↑（σ ×(1+0.8c)）**。

**验收（`--like-check`，同页面只改"喜欢"）**：像素变化 **73.9%**（阈值 >10%）；
**归一化预测误差 0.0394 → 0.0318（下降 19.3%）** → **PASS**。

> **度量的修正过程（重要，别重走弯路）**：最初用"相邻像素平均绝对差"当预测误差代理，三次调实现都 FAIL——
> 实测发现该量对本渲染器主要反映**结构/对比**（σ 收紧/展宽、去抖动都会抬高它），**不是**预测误差。
> 改用**邻域线性预测残差** `r = p[x] − (p[x−1]+p[x+1])/2` 并**除以对比度 std 做尺度归一化**后才有意义
> （单测证明：归一化残差对平移/缩放不变，而一阶梯度随对比度上升）。
> 又经**参数扫描**选定"平衡配置"（σ×0.8 + 抖动×0.85，−19.3%），
> 而不取"σ×1.6"那种下降 37.7% 但**主要靠糊**的配置。

### 9.2 L3 世界模型（载体定位，借鉴 Visionary）
内核新增 `l3_world.rs`（零依赖、+6 测试）：条目＝页面/站点/交互；`observe_*` 累积、`summary()` 可查询、
`to_text/from_text` 可持久化、**同输入序列必得同模型**（确定性）。
宿主：切标签/取源码/调四元组都 `observe_*`；**画面 = 当前页面场域 ⊕ 世界模型均值（权重 0.35）**；
下载时附 `world.txt` 快照；UI 侧栏显示条目/tick/主导/世界均值/一致性。
**验收（`--world-check`）**：更新后 7 条目、tick 5、往返一致；**画面变化 93.1%** → PASS。

### 9.3 动态 LOD（借鉴 SuperSplat）
`lod_n_for(紧张, 平静)` → `activity = 0.6·紧张 + 0.4·(1−平静)` → **n ∈ {128,256,512,1024}（2 的幂，适配双调排序）**；
切换时**重建管线**（旧缓冲即释放）并带滞回（跨两档才切）。
**验收（`--lod-check`）**：场域渲染缓冲 **147456 → 18432 字节（1/8.0 < 1/5）**；帧率随档位下降而升高；
档位随四元组单调 → PASS。窗口内实测自动切换：`LOD 切换 147456 → 18432 字节（n=128，趟数=28），5.83ms`。
