...
**本轮边界**：只交付本设计文档；实现（网关代码）按 A 类新模块走完整流水线（专家审核 → 实现 → CI → 验收），另起实现轮。应用框架、参考应用、L6 对齐层**不在本阶段展开**，但本设计须满足 §7 通用性要求以支撑其扩展。
...
| R5 | **接口契约稳定、语义开放** | 快照/事件 schema 版本化；事件类型集开放，供下游各层/应用扩展 |

## 3. 分层与接口形态路线

```
应用框架/参考应用/对齐层 ────────────── 本阶段不展开；预留扩展点（§7）
        │  经 L3 标准接口
...
> ⚠️ **本表为「设计稿的历史快照」，不是权威口径**（2026-09-18 裁定 Q12）。
> **端点表唯一权威＝`docs/LAYER_ARCHITECTURE.md §4.L3`（23 路径）**；本表**只保留下方 6 个 v0 基线端点的语义说明**，
> **新增/变更端点一律改权威表**，本表不再枚举全集。
...
## 7. 通用性要求（支撑下游各层/应用扩展，本阶段不展开）

1. **传输无关**：语义层与传输解耦——协议文档化的 JSON schema 即契约；二期 WebSocket / 未来 IPC 只换通道，不改语义；
2. **事件类型开放**：`state_change`/`instruction` 为已定义最小集；schema 版本化（`schema` 字段 + 兼容性规则：只增不改、旧版本可读），应用框架（npb-appkit）可挂新事件类型；
3. **tag 扩展位保留**（§4.3），为多应用接入预留隔离/审计钩子；
4. **快照字段来自单一真相源**（内核 FFI），任何应用所见即内核所是；
