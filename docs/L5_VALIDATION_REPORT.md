...
## 7. 真实采集验证（2015 老笔记本 · 192.168.1.4）2026-09-10 凌晨

- 链路：开发机（192.168.1.3）起 LAN 网关 → 笔记本浏览器/一键 bat 下载 cloud-probe.exe →
  `--report http://192.168.1.3:3000/v1/probe` 回传 → 网关 /v1/probe 存储 → L4/L5 链诊断。
- **七维场域数据（真实采集）**：`s = [0.099, 1.000, 1.15, 1.000, 1.294, 1.83, 6.587]`
  （t=CPU≈4% 空闲、f=平稳、a=内存占用 46%、φ=同步正常、x=磁盘余量 64.7%、H=进程 183、τ=线程 1976）
- **L4 戒律判定：Pass**（high=2/7：H 与 τ 越 1.618；不足 3 维 → 允许使用）✅
- **L5 号脉诊断（schema 2）**：
  - 四场分解 fields：earth=(x+τ)/2≈3.94 **亢**（连接/结构量高）、water=H=1.83 **亢**（进程量高）、
    fire=(a+t)/2≈0.62 平、wind 平
  - 主结论：EARTH 亢（结构连接相对健康基线越限）；cause=地行结构承压过载
  - 多语言 summary：universal="亢进·根基，亢进·缓冲"；hardware="过高/过载·结构/承载，
    过高/过载·余量/缓存"；tcm="实证/亢盛·土(脾胃)，实证/亢盛·水(肾水)"（词条命中）；
    user 含 (待补)（earth 口语词未验）
- **与主观感受对照**：用户感受="还行"。诊断显示 CPU 空闲（t≈0.1）、内存余量充足（46%）、
  L4 Pass——与"基本可用、日常不严重卡顿"一致 ✅；偏高项集中在进程/线程规模（H/τ），
  属老系统常驻后台多的常态——**本底场校准说明**：本次用健康启发基线（1.0）诊断；该机真实
  "本底"（常驻 180+ 进程/2000+ 线程）应经 learn 建立后二次对比才精准（后续采集积累即自动校准）。
- 附：修复项（真实采集暴露）——cloud-probe --report 端点兼容带路径（原仅 host:port，
  一键 bat 传 /v1/probe 曾致 invalid port；已修并本机复验回传闭环）。
